...
func splitSegment(p1, p2 Point) []Point {
	...
	if p1.x == p2.x {
		...
		return funk.Map(xs, func(x int) Point {
			if slope > 0 {
				return Point{x, miny + x - minx}
			} else {
				return Point{x, maxy - (x - minx)}
			}
		}).([]Point)

		/* code to find all points on the borders of one rectangle
		var all []Point
		{
			xs := makeRange(minx, maxx-1)
			pa := funk.Map(xs, func(i int) Point {
				return Point{i, miny}
			}).([]Point)
			all = append(all, pa...)
		}
		{
			ys := makeRange(miny, maxy-1)
			pb := funk.Map(ys, func(i int) Point {
				return Point{maxx, i}
			}).([]Point)
			all = append(all, pb...)
		}
		{
			xs := makeRange(maxx, minx+1)
			pc := funk.Map(xs, func(i int) Point {
				return Point{i, maxy}
			}).([]Point)
			all = append(all, pc...)
		}
		{
			ys := makeRange(maxy, miny+1)
			pd := funk.Map(ys, func(i int) Point {
				return Point{minx, i}
			}).([]Point)
			all = append(all, pd...)
		}
